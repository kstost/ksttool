...
ksttool.expand_rectangle = function (lm, rect) {
    /*
       lm를 { x: 10, y: 20 }
       let rect = [
           { x: 0, y: 0 },
    ...
    */
    ['x', 'y'].forEach(co => {
        rect.map(r => ({ val: r[co], body: r })).sort((a, b) => a.val - b.val).forEach((r, idx) => {
            let pv = (idx < 4 / 2) ? -lm[co] : lm[co];
            r.body[co] += pv;
        })
    })
};

ksttool.math.performance_check = function (key, SSS) {
    /*
    사용하기 위해서는 먼저 window.pmc = {} 를 해둔다
    그리고 측정할 코드 시작부에
    let SSS = window.pmc ? new Date() : null;
    측정종료시점에
    ksttool.math.performance_check('측정명', SSS)
    */
    if (window.pmc) {
        let vv = new Date() - SSS;
        if (!pmc[key]) { pmc[key] = 0; }; pmc[key] += vv;
        if (!pmc['_' + key]) { pmc['_' + key] = 0; }; pmc['_' + key] += 1;
    }
};

ksttool.checkDotIsInsidePolygon = function (poly, pt) {
    /*
        poly 폴리곤에 pt 점이 들어있나 판단해주는거다
        poly는 XYX방식으로 주면 된다
    */
    ...
    for (var c = false, i = -1, l = poly.length, j = l - 1; ++i < l; j = i)
        ((poly[i].y <= pt.y && pt.y < poly[j].y) || (poly[j].y <= pt.y && pt.y < poly[i].y))
            ...
            && (c = !c);
    // ksttool.math.performance_check('checkDotIsInsidePolygon', SSS)
    return c;
};


ksttool.math.make_vectors_two_point = function (center_point, ra, line_length, ratio) {
    // radian 각도의 기울기를 가진 line_length 길이의 선을 만들고 그 선의 ratio 에 해당하는 지점을 center_point 에 맞춘다
    // 설명이 어려우면 center_point 와 라인의 두점, 총 3개의 점을 찍어보면 쉽게 보인다
    // let SSS = pmc ? new Date() : null;
    // let ra = radian;
    let dist_f = (line_length * ratio);
    let dist_s = (line_length * (1 - ratio));
    let first = ksttool.math.get_coordinate_distance_away_from_center_with_radian(dist_f, center_point, ra)
    let second = ksttool.math.get_coordinate_distance_away_from_center_with_radian(dist_s, center_point, ra + Math.PI)
    // ksttool.math.performance_check('make_vectors_two_point', SSS)
    return { first, second };
}
...
ksttool.math.get_closest_point_by_sorting = function (list_need, circle_cdls) {
    ...
    circle_cdls 는 {x:0,y:0} 점
    -------------
    list_need 안에서 circle_cdls와 가장 근접한 점이 뭔지를 알아봐준다
    */
    // let SSS = pmc ? new Date() : null;
    var sorted_list;// = [];
    // var idx_list = [];
    let min;
    for (var i = 0; i < list_need.length; i++) {
        let len = ksttool.math.get_distance_between_two_point(circle_cdls, list_need[i].coordinate);
        if (min === undefined || min > len) {
            sorted_list = list_need[i].coordinate;
            min = len;
        }
    }
    // ksttool.math.performance_check('get_closest_point_by_sorting', SSS)
    return sorted_list;
};
ksttool.divide_screen = function (div, size, area_range) {
    // size 를 {width: 100, height:200} 으로 주고
    // div 를 { x: 2, y: 2 } 로 주면
    // 분할된 영역을 배열로 준다
    let rects = [];
    let width = size.width / div.x;
    let height = size.height / div.y;
    for (let x = 0; x < div.x; x++) {
        for (let y = 0; y < div.y; y++) {
            let rect = [
                { x: x * width, y: y * height },
                { x: x * width + width, y: y * height },
                { x: x * width + width, y: y * height + height },
                { x: x * width, y: y * height + height }
            ];
            if (area_range) {
                ksttool.expand_rectangle({ x: area_range, y: area_range }, rect);
            }
            rects.push(rect);
        }
    }
...
ksttool.convert_XYX_into_PLP = function (poly) {
    // xyx를 plp로 변환해줌
    // let SSS = pmc ? new Date() : null;
    let vecc = [];
    for (let i = 0; i < poly.length; i++) {
        let vec = { first: poly[i], second: poly[i + 1] };
        if (!vec.second) {
            vec.second = poly[0];
        }
        vecc.push(vec);
    }
    // ksttool.math.performance_check('convert_XYX_into_PLP', SSS)
    return vecc;
}

ksttool.math.is_circle_poly_intersect = function (circle, poly) {
    // 원과 폴리곤이 중첩되었는지 판별해주고
    // 원의 중심점이 폴리곤 안에있으면 1 리턴, 밖에 있으면서 중첩되있으면 2리턴 밖에있으면서 중첩되있지 않으면 falsy(null이였나 undefined였나 확실하지 않으니 찍어보자) 리턴
    // let SSS = pmc ? new Date() : null;
    let inin = ksttool.checkDotIsInsidePolygon(poly, { x: circle.x, y: circle.y });
    if (inin) { return 1; }
    poly.every((dot, idx) => {
        let vector = {
            first: dot,
        ...
        let df = ksttool.check_intersection_line_circle(vector, {
            x: circle.x,
            y: circle.y,
            r: circle.r
        });
